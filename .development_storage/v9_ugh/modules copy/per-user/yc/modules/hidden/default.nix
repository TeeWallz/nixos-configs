{ config, lib, ... }:
with lib;
let cfg = config.my.per-user.yc.modules.hiddenServices;
in {
  options.my.per-user.yc.modules.hiddenServices = {
    enable = mkOption {
      type = types.bool;
    ...
    resolv = mkOption {
      type = types.bool;
      default = config.my.per-user.yc.modules.hiddenServices.enable;
    };
  };
