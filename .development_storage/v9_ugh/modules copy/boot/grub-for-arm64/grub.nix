...
let
  ...
  cfg = config.my.boot.aarch64Grub;
  ...
  grubConfig = args:
    ...
    in pkgs.writeText "grub-config.xml" (builtins.toXML {
      ...
      grubTargetEfi = if cfg.efiSupport && (cfg.version == 2) then
        ...
        "";
      bootPath = args.path;
      storePath = config.my.boot.aarch64Grub.storePath;
...
in {
    ...
    my.boot.aarch64Grub = {
      ...
      device = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          use `my.boot.aarch64Grub.devices`.
        '';
      };
  ...
  config = mkMerge [
    ...
    {
      my.boot.aarch64Grub.splashImage = mkDefault (if cfg.version == 1 then
        pkgs.fetchurl {
          url =
    ...
    (mkIf (cfg.splashImage == defaultSplash) {
      my.boot.aarch64Grub.backgroundColor = mkDefault "#2F302F";
      my.boot.aarch64Grub.splashMode = mkDefault "normal";
    })

    (mkIf cfg.enable {

      my.boot.aarch64Grub.devices = optional (cfg.device != "") cfg.device;

      my.boot.aarch64Grub.mirroredBoots = optionals (cfg.devices != [ ]) [{
        path = "/boot";
        inherit (cfg) devices;
      ...
      my.boot.aarch64Grub.extraPrepareConfig = concatStrings (mapAttrsToList
        (n: v: ''
          ${pkgs.coreutils}/bin/cp -pf "${v}" "@bootPath@/${n}"
        '') config.my.boot.aarch64Grub.extraFiles);
      ...
      assertions = [
        ...
        }
        {
          ...
          message = "You must set the option ‘my.boot.aarch64Grub.devices’ or "
            + "'my.boot.aarch64Grub.mirroredBoots' to make the system bootable.";
        }
        {
        ...
        }
        {
          ...
          message =
            "If you wish to to use my.boot.aarch64Grub.efiInstallAsRemovable, then turn on my.boot.aarch64Grub.efiSupport";
        }
        {
          assertion = cfg.efiInstallAsRemovable
            -> !config.boot.loader.efi.canTouchEfiVariables;
          message =
            "If you wish to to use my.boot.aarch64Grub.efiInstallAsRemovable, then turn off boot.loader.efi.canTouchEfiVariables";
        }
